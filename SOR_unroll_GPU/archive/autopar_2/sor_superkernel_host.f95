module singleton_module_src_2_postcpp_sor_superkernel
 contains
 subroutine sor_superkernel(p0,p2,rhs)
...
! ---- BEGIN sor_superkernel_map_21 -------------------------------------------------------------------------------------------
 oclGlobalRange = (((101 - 0) + 1) * (((101 - 0) + 1) * ((81 - 0) + 1)))
 oclLocalRange = 0
 state_ptr(1) = ST_SOR_SUPERKERNEL_MAP_21
 ...
 ! call sor_superkernel_map_21

! ---- END --------------------------------------------------------------------------------------------------------------------
! ---- BEGIN sor_superkernel_map_45 -------------------------------------------------------------------------------------------
 oclGlobalRange = (((101 - 0) + 1) * (((101 - 0) + 1) * ((81 - 0) + 1)))
 oclLocalRange = 0
 state_ptr(1) = ST_SOR_SUPERKERNEL_MAP_45
 
 state_ptr_ptr(1) = state_ptr
 call oclWrite1DIntArrayBuffer(state_ptr_buf,state_ptr_ptr_sz,state_ptr_ptr)! Automatic conversion to array
 call runOcl(oclGlobalRange,oclLocalRange,exectime)
 ! call sor_superkernel_map_45

! ---- END --------------------------------------------------------------------------------------------------------------------
 end subroutine sor_superkernel
! Footer (produceCode_progUnit c)
end module singleton_module_src_2_postcpp_sor_superkernel
